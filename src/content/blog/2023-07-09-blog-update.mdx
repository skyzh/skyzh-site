...
description: "Chi has updated his blog to use the Astro web framework. You can now react to each post by clicking the reaction buttons at the end; the feature is powered by Vercel Edge Functions and Neon Postgres."
...
When deciding on a framework for my new personal website, Astro stood out for two primary reasons. First, I wanted
to explore a full-stack web application framework in JavaScript / TypeScript, and Astro provided the perfect opportunity
to do so. Second, Astro’s blog template offered easy customization and comprehensive RSS support, which was
crucial for my blogging needs.

<small>Fun fact: One facet of my experience often goes unnoticed: in addition to my more recent Rust + database background, I have been involved in web development for 9 years, beginning with my first PHP project in 2014. That project was a Codeforces-style online judge for internal use.</small>

## Project Setup

I started my project using the `npm create astro` command with the minimal blog template.
...
a slight issue: I was using TOML front matter, while Astro only supports YAML. I had to make some minor
...
assets, so I took a different approach and placed them directly in the public directory. This way, the
...
One exciting feature I’ve been diligently working on for my new personal website is the integration of reactions into
...
[Neon Postgres](https://neon.tech) offers a fully managed Postgres service (from the company I currently work for... 🤪).
...
to retrieve and update reaction data. Both Neon and Vercel provide generous *FREE* tiers, enabling me to build and
implement this feature without paying a dollar.

## API Design and Deployment to Vercel Edge Functions
...
As the names suggest, the `get` API retrieves the reaction counts for a specific blog post, and the `add` API
adds a reaction to a specific blog post. The `slug` parameter is the unique identifier for each blog post.
...
Postgres isn’t feasible. The runtime only supports HTTP / WebSocket. Therefore, I incorporated the [Neon Serverless Driver](https://github.com/neondatabase/serverless)
...
list of existing blog posts in the `src/content/blog` directory. A JSON file containing all blog posts on the website
...
will read this file and populate the database. Users are only allowed to modify existing rows rather than add new ones
through the Vercel Edge Function.

## Client-Side Rendering

Lastly, I need to render the reaction buttons on the client side. React handles this in the web browser.
To enable client-side rendering in Astro, I need to add the React integration:
...
And there you have it, the new beginning of my personal website! I hope you enjoyed reading this article, and you can click the
reaction buttons below ⬇️ to share your instant feedback. As usual, I have created a GitHub
