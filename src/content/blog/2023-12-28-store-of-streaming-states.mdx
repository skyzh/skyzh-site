...
For example, when a user executes the following query on a previously created view:

```
SELECT * FROM result WHERE category = "Vegetable"
```

The computation of the streaming system is triggered. During the computation, only the data related to `category = "Vegetable"` is processed and the relevant state is stored. Using this query as an example, we will explain Noria's computation method and state storage.
