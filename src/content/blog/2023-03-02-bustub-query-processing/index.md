...
Before Fall 2022, the [BusTub](https://github.com/cmu-db/bustub) project (a course project in CMU 15-445/645 Database Systems) only covered certain aspects of database systems: memory management (Project 1 Buffer Pool Manager), storage engines (Project 2 Index), query execution (Project 3 Query Execution), and concurrency control (Project 4 Concurrency Control). In August 2022, I began writing the query processing layer for BusTub, which introduced a complete SQL layer and enabled BusTub to make the leap to a full-fledged SQL database. Starting in Fall 2022, students could use BusTub directly to run SQL queries and verify the correctness of their operator implementations.

![BusTub Overview](overview.png)

BusTub’s SQL layer takes a similar approach to the [RisingLight](https://github.com/risinglightdb/risinglight) project that I previously participated in, as well as to [DuckDB](https://github.com/duckdb/duckdb). The parser uses the libpg_query library, packaged by DuckDB, to parse SQL statements. After parsing, the binder binds identifiers to various entities, the planner generates a query plan, and the optimizer optimizes it to produce the final query plan. Currently, the entire SQL engine of BusTub supports simple join and aggregation queries, uncorrelated subqueries, and CTEs. It is worth mentioning that BusTub can now also be compiled into WASM and run directly [in the browser](https://15445.courses.cs.cmu.edu/spring2023/bustub/). We have distributed the compiled artifacts of the BusTub reference solution through a web page so that students can get a rough idea of what they can achieve at the end of the semester before starting the project.

*Fun fact: All database systems with binders are more or less connected to CMU, where binders seem to have first appeared in the [Peloton](https://github.com/cmu-db/peloton) project.*
...
This article mainly shares experiences, so most of the content is “facts” about the BusTub SQL layer. Not many people write binders, planners, and optimizers from scratch in the database industry. If someone has the opportunity to do so, they can look at the pitfalls that I encountered and perhaps learn something from this article.

Below, I introduce the various modules of the BusTub SQL engine. The parser simply uses libpg_query, so there isn’t much to say about it in detail, and I will skip this part.

## Binder

After generating the Postgres AST through libpg_query, the Binder rewrites this AST into a higher-level AST that BusTub can understand. In this process, it resolves all identifiers to entities. Let’s take the simplest example of `select *`:
...
Planner recursively traverses the BusTub AST generated by Binder to generate a preliminary query plan. To make the implementation easier and more understandable, BusTub has a few design points:
...
* **There is no distinction between logical plan nodes and physical plan nodes.** BusTub’s SQL layer has a planner and an optimizer. Typically, in this design, the planner generates logical plan nodes, which are then optimized through a series of steps to produce physical plan nodes. However, BusTub is just a teaching project, so we only have physical plan nodes. The planner directly generates a NestedLoopJoin plan, and then the optimizer rewrites it as a HashJoin or NestedIndexJoin.
* **We added filter, projection, and values plan nodes.** Previously, every executor needed to support projections and filters, filtering or projecting its output before returning a tuple to its parent executor. These are now separate operators. Also, the values plan node is used for the `insert into values ...` statement.
...
Previously, BusTub processed the entire query with a single aggregation operator. Under the new SQL layer design, we chose to split it up so that each operator is responsible for only one thing. Now, this query generates the following execution plan in the planner stage:
...
This way, the responsibilities of each operator become quite clear. But how exactly is this planning process done? When the planner sees the expression `max(y) + min(z)`, it initially sees only a binary `+` operation. Only after looking one level deeper at the left- and right-hand sides can it identify that this addition must be performed after the aggregation.
...
Once this process is complete, we can finish planning the entire aggregation based on the aggregate list and the rewritten select list/having clause.
...
The query execution project ran well in Fall 2022. The writeup was complete. Test cases were redesigned to use SQL. Few students came to office hours, and Piazza was quiet. We received feedback from students that it was too easy compared with Project 2 (B+ tree index) and Project 4 (lock manager / concurrency control), probably because using SQL is more intuitive than working on projects where students need to debug concurrency issues. Therefore, in Spring 2023, we added more optimizer work to the project, where students needed to implement hash joins with composite join-key support and the rule to convert NLJ to a hash join.
