...
When I first looked at the BusTub repo in the summer of 2022, I was surprised that BusTub did not support SQL, even though the entire course was based on relational databases (and SQL)! The lack of SQL support was a big problem because students could not learn how a database system works end to end by solely staring at query plans. Therefore, I took the initiative to add a query processing layer to BusTub. After some initial investigation by [Garrison](https://www.garrisonhess.com), we realized that the easiest way to integrate SQL was to build the layer from scratch instead of plugging in a query processing layer from another system. There was a branch that made BusTub the query backend of PostgreSQL, but it was far from complete. Following DuckDB’s approach, I used only the PostgreSQL parser and wrote the rest of the query processing layer (i.e., binder, planner, and optimizer) myself.
